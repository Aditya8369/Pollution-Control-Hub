self.onmessage = function (e) {
  ...
  processedDays.forEach((day) => {
    ...
    if (!monthlyAverages[month])
      monthlyAverages[month] = { sum: 0, count: 0, max: -Infinity };
    monthlyAverages[month].sum += day.avgAqi;
    monthlyAverages[month].count += 1;
    if (day.maxAqi > monthlyAverages[month].max)
      monthlyAverages[month].max = day.maxAqi;
  });

  const formattedMonthly = Object.keys(monthlyAverages)
    .sort()
    .map((month) => ({
      month,
      avgAqi: Math.round(
