 ...
 * Module: Noise Pollution Cards UI Component Tier
 * File: src/components/NoiseCards.jsx
 * Domain: React Functional UI Components, Modular Acoustic Telemetry Cards
 */

import React from 'react';
import { getNoiseExposureSeverity } from './noiseTypes';
...
export function NoiseSourceCard({ source, selectedDistance = 10 }) {
  const severity = getNoiseExposureSeverity(source.avgDba);
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2 text-xs font-mono mb-3">
          <div className="bg-slate-950/60 p-2 rounded border border-slate-800">
            <span className="text-slate-500 block text-[10px]">AVG DECIBEL</span>
            <span className="text-amber-400 font-bold text-base">{source.avgDba} dBA</span>
          </div>
          <div className="bg-slate-950/60 p-2 rounded border border-slate-800">
            <span className="text-slate-500 block text-[10px]">PEAK PEAK</span>
            <span className="text-red-400 font-bold text-base">{source.maxDba} dBA</span>
          </div>
        </div>
      </div>

      <div className="text-[11px] font-mono text-slate-400 bg-slate-950/40 p-2 rounded border border-slate-800/60">
        WHO Status: <span className={source.avgDba > 65 ? 'text-red-400 font-bold' : 'text-emerald-400 font-bold'}>
          {source.avgDba > 65 ? 'Exceeds Quiet Zone Limit' : 'Within Urban Standard'}
        </span>
      </div>
    </div>
  );
...
export function NoiseZoneCard({ zone }) {
  const severity = getNoiseExposureSeverity(zone.baselineDba);
    ...
    <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800 flex flex-col justify-between">
      <div>
        <div className="flex justify-between items-center mb-2">
          ...
          <span className="text-xs font-mono font-bold" style={{ color: severity.color }}>{zone.baselineDba} dBA</span>
        </div>
...
export function NoiseGoalCard({ goal }) {
  ...
  return (
    <div className={`p-4 rounded-xl border ${
      ...
      <div className="flex justify-between items-baseline text-xs font-mono my-3">
        ...
        <span className="font-bold text-slate-200">{goal.currentDba} dBA / <span className="text-emerald-400">{goal.targetDba} dBA</span></span>
      </div>

      <div className="w-full bg-slate-950 h-2 rounded-full overflow-hidden border border-slate-800">
        <div 
          className={`h-full transition-all ${isAchieved ? 'bg-emerald-500' : 'bg-amber-500'}`}
          style={{ width: `${Math.min(100, (goal.targetDba / goal.currentDba) * 100)}%` }}
        />
      </div>
    </div>
  );
