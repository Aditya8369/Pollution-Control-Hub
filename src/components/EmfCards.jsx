 ...
 * Module: Urban Ambient EMF Cards UI Component Tier
 * File: src/components/EmfCards.jsx
 * Domain: React Functional UI Components, Electromagnetic Radiation Telemetry Cards
 */

import React from 'react';
import { getEmfRiskSeverity } from './emfTypes';
...
export function EmfSourceCard({ source }) {
  const severity = getEmfRiskSeverity(source.avgPowerDensityWm2);
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-2 text-xs font-mono mb-3">
          <div className="bg-slate-950/60 p-2 rounded border border-slate-800">
            <span className="text-slate-500 block text-[9px]">POWER DENS</span>
            <span className="text-cyan-400 font-bold text-sm">{source.avgPowerDensityWm2} W/m²</span>
          </div>
          <div className="bg-slate-950/60 p-2 rounded border border-slate-800">
            <span className="text-slate-500 block text-[9px]">FREQUENCY</span>
            <span className="text-amber-400 font-bold text-sm">{source.frequencyMhz} MHz</span>
          </div>
          <div className="bg-slate-950/60 p-2 rounded border border-slate-800">
            <span className="text-slate-500 block text-[9px]">MAG FIELD</span>
            <span className="text-purple-400 font-bold text-sm">{source.magneticFieldUt} µT</span>
          </div>
        </div>
      </div>

      <div className="text-[11px] font-mono text-slate-400 bg-slate-950/40 p-2 rounded border border-slate-800/60">
        ICNIRP Status: <span className={source.avgPowerDensityWm2 >= 10.0 ? 'text-red-400 font-bold' : 'text-emerald-400 font-bold'}>
          {source.avgPowerDensityWm2 >= 10.0 ? 'Exceeds 10 W/m² Public Limit' : 'Within ICNIRP Guideline'}
        </span>
      </div>
    </div>
  );
...
export function EmfZoneCard({ zone }) {
  const severity = getEmfRiskSeverity(zone.baselinePowerDensity);
    ...
    <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800 flex flex-col justify-between">
      <div>
        <div className="flex justify-between items-center mb-2">
          ...
          <span className="text-xs font-mono font-bold" style={{ color: severity.color }}>{zone.baselinePowerDensity} W/m²</span>
        </div>
      ...
      <div className="mt-4 pt-3 border-t border-slate-800/80 flex justify-between items-center text-xs font-mono">
        <span className="text-slate-400">EMF Exposure:</span>
...
export function EmfGoalCard({ goal }) {
    ...
    <div className={`p-4 rounded-xl border ${
      ...
      <div className="flex justify-between items-start mb-2">
        ...
        <span className={`px-2 py-0.5 text-[10px] font-mono rounded font-bold ${
          isAchieved ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/40' : 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/40'
        }`}>
          {goal.status}
        </span>
      </div>
      ...
      <div className="flex justify-between items-baseline text-xs font-mono my-3">
        <span className="text-slate-400">Current vs Target W/m²:</span>
        <span className="font-bold text-slate-200">{goal.currentPowerDensity} W/m² / <span className="text-emerald-400">{goal.targetPowerDensity} W/m²</span></span>
      </div>

      <div className="w-full bg-slate-950 h-2 rounded-full overflow-hidden border border-slate-800">
        <div 
          className={`h-full transition-all ${isAchieved ? 'bg-emerald-500' : 'bg-cyan-500'}`}
          style={{ width: `${Math.min(100, (goal.targetPowerDensity / goal.currentPowerDensity) * 100)}%` }}
        />
      </div>
    </div>
  );
