 ...
 */
export function localHourStamp(utcOffsetSeconds: number = 0): string {
...
): { index: number; exact: boolean; timestamp: string | null } {
 ...
 */
function getCurrentHourIndex(times: (string | null | undefined)[], utcOffsetSeconds: number = 0): number {
 ...
 * @type {AQIBand}
 */
export const UNKNOWN_AQI_BAND: AQIBand = { label: 'Unknown', color: '#94a3b8' };
 ...
 */
export function getAQIBand(value: number | null | undefined): AQIBand {
 ...
 */
export function getPollutantColor(value: number | null | undefined, limit: number): string {
 ...
 */
export async function fetchLocalGrid(
  lat: number,
  lon: number,
  topN: number = 6,
  signal?: AbortSignal
...
function computeConfidence(
  ...
  times: (string | null | undefined)[]
): { confidenceScore: 'High' | 'Medium' | 'Low'; dataCompleteness: number } {
 ...
 */
function readingAt(series: any, idx: number): number | null {
...
export async function fetchAirQualityByCoords(
   ...
   */
  const fetchWithWorker = (workerUrl: string, workerSignal?: AbortSignal): Promise<any> => {
...
export async function fetchSunSafetyData(
  lat: number,
  lon: number,
  signal?: AbortSignal
): Promise<{ temperature: number; feelsLike: number; uvIndex: number } | null> {
  if (!isValidCoord(lat, lon)) return null;
 ...
 */
interface EstimatedAverages {
  weekly: number | null;
  monthly: number | null;
  prediction: number | null;
}

export function estimateWeeklyMonthlyAverages(
  trend: Array<{ us_aqi?: number | null }> | null | undefined
): EstimatedAverages {
 ...
 */
export function subAqi(concentration: number, breakpoints: Breakpoint[]): number {
 ...
 */
export function estimateAQI(pm25: number, pm10: number, no2: number, o3: number, co: number): number {
  const scores = [
    subAqi(pm25, BP_PM25),
    subAqi(pm10, BP_PM10),
    subAqi(no2, BP_NO2),
...
export async function get7DayForecast(lat: number, lon: number, signal?: AbortSignal): Promise<ForecastDay[]> {
...
export async function fetchPollenData(lat: number, lon: number, signal?: AbortSignal): Promise<PollenData | null> {
  if (!isValidCoord(lat, lon)) return null;

  const getDeterministicFallback = (latitude: number, longitude: number): PollenData => {
...
export function getPollenSeverity(
  type: 'tree' | 'grass' | 'weed' | string,
